...
hybicon.prototype.createIcon = function () {
    ...
    if (this.infoRight) {
        this.raphael.setViewBox(0, 0, 300, 100, true);
    }
    else {
    ...
    if (this.hoverMode || this.clickMode) {
    ...
    if (this.infoMode || this.infoRight) {
        if (this.infoMode) {
        ...
        this.infotext.attr({ font: this.infoFont });
...
hybicon.prototype.clickHandler = function () {
    if (this.iconHref !== null) {
        if (this.iconHrefTarget === null) { this.iconHrefTarget = '_self'; }
        window.open(this.iconHref, this.iconHrefTarget)
    }
};
...
hybicon.prototype.parseIcon = function (holderDiv) {
    if (holderDiv !== undefined &&
        ...
        if (hybiconHasData) {
            ...
            //data-hybicon-inforight
            var hybiconInforight = holderDiv.getAttribute("data-hybicon-inforight");
            if (hybiconInforight !== null) {
                this.infoRight = true;
            }
            ...
            if (!this.clickMode) {
                //parse inner <a> tag in hybicon's div for use href in navigateFunction
                var foundHref = false;
                for (var i = 0; i < holderDiv.children.length; i++) {
                    if (holderDiv.children[i].getAttribute('href') !== undefined) {
                        this.iconHref = holderDiv.children[i].getAttribute('href');
                        this.iconHrefTarget = holderDiv.children[i].getAttribute('target');
                    }
                }
            }
...
hybicon.prototype.setDefaultProps = function () {
    ...
    // icon1
    if (this.icon1Size === null) { this.icon1Size = 77; }
...
hybicon.prototype.setIcon = function (iconName) {
    ...
    switch (iconName) {
        ...
        case "user-idea-change":
            this.icon2Size = 35;
            this.icon2SizeAnim = 80;
            this.icon1Size = 85;
            this.icon1SizeAnim = 35;
            this.icon1CenterX = 47;
            this.icon1CenterY = 55;
            this.icon2CenterX = 75;
            this.icon2CenterY = 24;
            this.icon1CenterXAnim = 75;
            this.icon1CenterYAnim = 24;
            this.icon2CenterXAnim = 47;
            this.icon2CenterYAnim = 54;
        case "user-idea":
...
var icon = {
    ...
    //Author of paper icon: Situ Herrera http://rivolta.es/
    paper: "M31.984,1.005c0-0.037,0-0.072-0.004-0.107s-0.003-0.07-0.012-0.104c-0.005-0.021-0.016-0.038-0.02-0.057c-0.012-0.037-0.023-0.073-0.038-0.108c-0.009-0.021-0.012-0.043-0.021-0.064c-0.019-0.03-0.037-0.058-0.059-0.088c-0.018-0.029-0.037-0.057-0.058-0.083c-0.03-0.038-0.062-0.076-0.096-0.11c-0.009-0.008-0.013-0.018-0.02-0.025c-0.016-0.014-0.035-0.017-0.051-0.029c-0.019-0.015-0.034-0.028-0.052-0.041c-0.015-0.01-0.021-0.023-0.033-0.032c-0.004-0.003-0.009-0.003-0.015-0.005c-0.007-0.005-0.011-0.011-0.018-0.014c-0.038-0.023-0.081-0.032-0.121-0.049c-0.032-0.014-0.062-0.028-0.096-0.038c-0.03-0.011-0.062-0.019-0.091-0.024c-0.036-0.008-0.07-0.008-0.107-0.012C31.039,0.009,30.995,0,30.95,0c-0.013,0.002-0.024,0.007-0.036,0.008c-0.021,0.001-0.047,0.009-0.072,0.013c-0.037,0.006-0.069,0.01-0.105,0.02c-0.041,0.012-0.08,0.024-0.119,0.042c-0.031,0.013-0.062,0.028-0.091,0.046c-0.026,0.014-0.057,0.023-0.081,0.04c-0.007,0.005-0.017,0.007-0.021,0.011h-0.001L0.429,21.172c-0.357,0.25-0.512,0.704-0.381,1.12c0.131,0.415,0.519,0.698,0.953,0.698h10v8c0,0.037,0.019,0.068,0.021,0.104c0.008,0.069,0.02,0.135,0.04,0.2c0.021,0.062,0.046,0.116,0.074,0.172c0.031,0.056,0.064,0.104,0.105,0.154c0.044,0.054,0.093,0.099,0.147,0.141c0.027,0.023,0.045,0.056,0.074,0.074c0.031,0.021,0.065,0.026,0.098,0.043c0.031,0.017,0.062,0.029,0.095,0.043c0.113,0.041,0.227,0.069,0.344,0.069h0.003c0.119-0.002,0.234-0.028,0.347-0.07c0.022-0.009,0.047-0.018,0.069-0.027c0.103-0.045,0.195-0.104,0.277-0.187c0.014-0.015,0.022-0.029,0.036-0.043c0.04-0.045,0.081-0.086,0.113-0.14l3.614-5.709c0.022,0.017,0.038,0.039,0.062,0.053l11,6.001c0.295,0.157,0.647,0.164,0.945,0.006c0.298-0.156,0.494-0.451,0.527-0.785l2.99-29.99C31.992,1.066,31.984,1.037,31.984,1.005zM11.56,20.99H4.175L24.306,6.899L11.56,20.99z M13.001,27.543v-5.166l10.902-12.051L13.001,27.543z M27.157,29.391l-9.632-5.252L29.58,5.102L27.157,29.391z"
};
